...
sudo apt-get update
sudo apt-get install mpc python-dev python-rpi.gpio python-pip htop elinks evtest tslib libts-bin python-gtk2
...
append "options fbtft_device name=adafruitts rotate=90 frequency=32000000" /etc/modprobe.d/adafruit.conf
...
if [ -z "$( grep "echo 252 > /sys/class/gpio/export" /etc/rc.local )" ]; then
    ...
    append "DISPLAY=:0.0 python /home/pi/screen_control.py &" /etc/rc.local
...
cd Pi_Time
sudo cp /home/pi/usr\ local\ bin/* /usr/local/bin/
